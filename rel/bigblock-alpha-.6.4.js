/*global BigBlock, window, navigator */
/**
 * Big Block Framework
 ...
 * 
 */	
BigBlock.getFullGridIndex = function (x, y) { // !! do not need this; use getPixIndex
...
BigBlock.getBigBlockCSS = function (id) {
	...
	for (i = 0, i_max = c.length; i < i_max; i += 1) {
		if (c[i].cssRules) { // Mozilla
			rules = c[i].cssRules;
			for (x = 0, x_max = rules.length; x < x_max; x += 1) {
...
BigBlock.checkHasDupes = function (A) {
	...
	for (i = 0; i < n; i += 1) { // outer loop uses each item i at 0 through n
		for (j = i + 1; j < n; j += 1) { // inner loop only compares items j at i+1 to n
	...
	return {
		...
		renderChar: function (id, obj, color, offset) {
			...
			if (BigBlock.ptInsideRect(x, y, 0, BigBlock.Grid.width, 0, BigBlock.Grid.height) && y < BigBlock.GridStatic.height) { // check that block is on screen
				...
				//this.setBlockAttribute(child, 'id', BigBlock.getUniqueId()); // uncomment to give each text div a unique id
