...
BigBlock.BlockBig = (function () {
	...
	return {
		...
		create: function (params) {
			...
			i = 0;
			for (h = 0; h < Math.floor(params.height/BigBlock.Grid.blk_dim); h++) { // loop thru height
				for (w = 0; w < Math.floor(params.width/BigBlock.Grid.blk_dim); w++) { // loop thru width
...
BigBlock.Character = (function () {
	...
	return {
		...
		create: function (params) {
			...
			for (i = 0; i < pal.classes.length; i++) { // get length of color palette for this color
...
BigBlock.Emitter = (function () {
	...
	return {
		...
		create: function (params) {
			var i;
			var F = BigBlock.clone(BigBlock.Block);  // CLONE Word
			var obj = new F;
		...
		add: function(x, y, backgroundColor){
			...
			try {
				if (typeof(x) === "undefined" || typeof(y) === "undefined") {
					throw new Error("BigBlock.Footer.add(): BigBlock.Footer.add(x, y) requires both an x and y coordinate.");
				} else {
		...
		setStyle : function (key, value) { 
			try {
				if (typeof(key) === "undefined") {
					throw new Error("BigBlock.Footer.setStyle(): BigBlock.Footer.setStyle(key, value) requires a style key.");
				}
				if (typeof(value) === "undefined") {
					throw new Error("BigBlock.Footer.setStyle(): BigBlock.Footer.setStyle(key, value) requires a style value.");
				}				
			} catch(e) {
					BigBlock.Log.display(e.name + ": " + e.message);
			}
...
BigBlock.Header = (function () {
	...
	return {
		...
		setStyle : function (key, value) { 
			try {
				if (typeof(key) === "undefined") {
					throw new Error("BigBlock.Header.setStyle(): BigBlock.Header.setStyle(key, value) requires a style key.");
				}
				if (typeof(value) === "undefined") {
					throw new Error("BigBlock.Header.setStyle(): BigBlock.Header.setStyle(key, value) requires a style value.");
				}				
			} catch(e) {
					BigBlock.Log.display(e.name + ": " + e.message);
			}
			...
			if (!this.is_paused) {
				...
				for (i = 0; i < BigBlock.BlocksKeys.length; i++) {
					if (typeof(BigBlock.Blocks[BigBlock.BlocksKeys[i]]) !== "undefined") {
						...
						if (obj.is_anim_updated) {
							is_anim_updated	= true;
						}
					}			
				}										
				...
				if (this.blocks_to_destroy.length > 0 || is_anim_updated) {
	...
	return {
		create: function (params) {
			...
			obj.destroy = function (callback) {// overwrite the Block's destroy function; include function to remove associated Character from the Text Grid
				...
				var i, q, word_id, quads, nodes, tmp, x;
				...
				for (i = 0; i < quads.length; i++) {
					...
					if (q.hasChildNodes()) {
						...
						for (x = 0; x < tmp.length; x++) { // loop thru children
