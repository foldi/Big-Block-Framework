		...
		add: function(bg_pix){
			...
			try{
				if (typeof(bg_pix) != 'object') {
					throw new Error("To render a background, you must provide a valid array of json objects in the following format: [{'c':c,'i': i}, {}, {}].");
				} else {
		...
		setBodyStyle : function (key, value) { 
			try {
				if (typeof(key) == 'undefined') {
					throw new Error('BigBlock.Timer.setBodyStyle(key, value) requires a style key.');
				}
				if (typeof(value) == 'undefined') {
					throw new Error('BigBlock.Timer.setBodyStyle(key, value) requires a style value.');
				}				
			} catch(e) {
					BigBlock.Log.display(e.name + ': ' + e.message);
			}
...
BigBlock.Footer = (function () {
	...
	return {
		...
		add: function(x, y, backgroundColor){
			...
			try {
				if (typeof(x) == 'undefined' || typeof(y) == 'undefined') {
					throw new Error("Footer.add(x, y) requires both an x and y coordinate.");
				} else {
		...
		setStyle : function (key, value) { 
			try {
				if (typeof(key) == 'undefined') {
					throw new Error('BigBlock.Footer.setStyle(key, value) requires a style key.');
				}
				if (typeof(value) == 'undefined') {
					throw new Error('BigBlock.Footer.setStyle(key, value) requires a style value.');
				}				
			} catch(e) {
					BigBlock.Log.display(e.name + ': ' + e.message);
			}
...
BigBlock.Grid = (function () {
	...
	return {
		...
		setStyle : function (key, value) { 
			try {
				if (typeof(key) == 'undefined') {
					throw new Error('BigBlock.Grid.setStyle(key, value) requires a style key.');
				}
				if (typeof(value) == 'undefined') {
					throw new Error('BigBlock.Grid.setStyle(key, value) requires a style value.');
				}				
			} catch(e) {
					BigBlock.Log.display(e.name + ': ' + e.message);
			}
		...
		add: function(){
			...
			for (var i = 0; i < 4; i++) {
				document.styleSheets[document.styleSheets.length - 1].insertRule("div#" + this.quads[i].id + " {background-color:transparent;width: " + BigBlock.Grid.width/2 + "px;height: " + BigBlock.Grid.height/2 + "px;position: absolute;left: " + ((window.innerWidth / 2) - (this.quads[i].left)) + "px;top: " + ((window.innerHeight / 2) - (this.quads[i].top)) + "px;z-index: " + this.quads[i].zIndex + "}", 0);
			}
			
			this.x = ((window.innerWidth / 2) - (this.quads[0].left)); // set the Grid x, y to the first quad's x, y
			this.y = ((window.innerHeight / 2) - (this.quads[0].top));
		...
		build : function () {
			...
			BigBlock.Loader.update({
				'total' : Math.round((document.styleSheets[document.styleSheets.length-1].cssRules.length / (Math.round(quad_width/BigBlock.Grid.pix_dim) * Math.round(quad_height/BigBlock.Grid.pix_dim))) * 100)
			});
			
			end_time = new Date().getTime();
			
			if (document.styleSheets[document.styleSheets.length - 1].cssRules.length < (Math.round(quad_width/BigBlock.Grid.pix_dim) * Math.round(quad_height/BigBlock.Grid.pix_dim))) { // if all styles are not complete, send false back to Timer; Timer will call build again
				return false;
			} else {
				return true;
			}
...
BigBlock.Header = (function () {
	...
	return {
		...
		add: function(x, y, backgroundColor){
			...
			try {
				if (typeof(x) == 'undefined' || typeof(y) == 'undefined') {
					throw new Error("Header.add(x, y) requires both an x and y coordinate.");
				} else {
					var d = document.createElement('div');
					d.setAttribute('id', this.alias);
					document.body.appendChild(d);
		...
		setStyle : function (key, value) { 
			try {
				if (typeof(key) == 'undefined') {
					throw new Error('BigBlock.Header.setStyle(key, value) requires a style key.');
				}
				if (typeof(value) == 'undefined') {
					throw new Error('BigBlock.Header.setStyle(key, value) requires a style value.');
				}				
			} catch(e) {
					BigBlock.Log.display(e.name + ': ' + e.message);
			}
...
BigBlock.ScreenEvent = (function () {
	return {
		...
		create: function (params) {
			...
			try {
				if (typeof(params) == 'undefined') {
					throw new Error('BigBlock.ScreenEvent.create(params) requires a params passed in json format. params can be empty ie. {}');
				} else {
					...
					document.addEventListener('click', this.click_event, false); // add click event listener
					...
					if (typeof(params.touchstart) != 'undefined') {
						...
						this.touch_event_start = function(event){
							...
							console.log('start: ' + this.touch.clientX + ', ' + this.touch.clientY);
						};
					...
					if (typeof(params.touchend) != 'undefined') {
						...
						this.touch_event_end = function(event){
							...
							//console.log('end: ' + this.touch.clientX + ', ' + this.touch.clientY);
...
BigBlock.Timer = (function () {
	...
	return {
		...
		play : function(params) { // called after all objects are ready
			...
			document.styleSheets[document.styleSheets.length - 1].insertRule("body {margin: 0;padding: 0;}", 0); // hard code no margin/padding for body
			...
			document.styleSheets[document.styleSheets.length - 1].insertRule("div.pix {float:left;width:" + BigBlock.Grid.pix_dim + "px;height:" + BigBlock.Grid.pix_dim + "px;position:absolute;}", 0);
		...
		build_char : function () {
			...
			if (BigBlock.GridActive.build_char_styles()) {
		...
		build_grid : function(){
			...
			if (BigBlock.GridActive.build()) {
				...
				//alert(new Date().getTime() - getBuildStart() + 'ms');
				...
				if (typeof(BigBlock.Sprites[obj.index]) == "object") {
					...
					try{
						if (BigBlock.Sprites[obj.index].render_static) {
							//throw new Error('You cannot kill static Sprites. Object alias: ' + BigBlock.Sprites[obj.index].alias);
						} else {
...
BigBlock.removeStatic = function (alias, callback) { 
	...
	try {
		...
		if (typeof(alias) != 'string') {
			throw new Error('BigBlock.removeStatic(alias); alias must be a string.');
		}		
	} catch(e) {
			BigBlock.Log.display(e.name + ': ' + e.message);
	}	
		...
		create: function (params) {
			...
			try {
				if (typeof(BigBlock.CharPresets.getChar) == 'undefined') {
					throw new Error('BigBlock.CharPresets = BigBlock.CharPresets.install() must be run before creating Words.');
				} else {
