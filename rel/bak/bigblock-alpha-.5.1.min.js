/* 
Big Block Framework
Copyright 2010 Foldi, LLC
This code is intended for use on ittybitty8bit.com and is not meant for redistribution.
*/
...
if(typeof a=="undefined")throw Error("Err: GSS002");}catch(e){BigBlock.Log.display(e.name+": "+e.message)}for(var c in this.quads)if(this.quads.hasOwnProperty(c))document.getElementById(this.quads[c].id).style[b]=a},add:function(){this.build_start_time=(new Date).getTime();var b,a;try{if(window.innerWidth){b=window.innerWidth;a=window.innerHeight}else if(document.documentElement.clientWidth){b=document.documentElement.clientWidth;a=document.documentElement.clientHeight}else{b=800;a=600;throw Error("Err: GA001");
}}catch(e){BigBlock.Log.display(e.name+": "+e.message)}for(var c=document.styleSheets.length-1,f=0;f<4;f++)try{if(document.styleSheets[c].insertRule)document.styleSheets[document.styleSheets.length-1].insertRule("div#"+this.quads[f].id+" {background-color:transparent;width: "+BigBlock.Grid.width/2+"px;height: "+BigBlock.Grid.height/2+"px;position: absolute;left: "+(b/2-this.quads[f].left)+"px;top: "+(a/2-this.quads[f].top)+"px;z-index: "+this.quads[f].zIndex+"}",0);else if(document.styleSheets[c].addRule)document.styleSheets[document.styleSheets.length-
1].addRule("div#"+this.quads[f].id,"background-color:transparent;width: "+BigBlock.Grid.width/2+"px;height: "+BigBlock.Grid.height/2+"px;position: absolute;left: "+(b/2-this.quads[f].left)+"px;top: "+(a/2-this.quads[f].top)+"px;z-index: "+this.quads[f].zIndex);else throw Error("Err: GA002");}catch(g){BigBlock.Log.display(g.name+": "+g.message)}this.x=b/2-this.quads[0].left;this.y=a/2-this.quads[0].top;for(f in this.quads)if(this.quads.hasOwnProperty(f)){b=document.createElement("div");b.setAttribute("id",
this.quads[f].id);document.body.appendChild(b);for(var h in this.styles)if(this.styles.hasOwnProperty(h))document.getElementById(this.quads[f].id).style[h]=this.styles[h]}},build:function(){for(var b=0,a=BigBlock.Grid.width/2,e=BigBlock.Grid.height/2,c=Math.round(e/BigBlock.Grid.pix_dim)/2,f=document.styleSheets.length-1,g=0;g<Math.round(a/BigBlock.Grid.pix_dim)*Math.round(e/BigBlock.Grid.pix_dim)/c;g++){if(b<Math.round(a/BigBlock.Grid.pix_dim))b++;else b=1;g%Math.round(a/BigBlock.Grid.pix_dim)===
0&&this.build_rowNum++;try{if(document.styleSheets[f].insertRule)document.styleSheets[document.styleSheets.length-1].insertRule(".pos"+(g+this.build_offset)+" {left:"+(b-1)*this.pix_dim+"px;top:"+(this.build_rowNum-1)*this.pix_dim+"px;}",0);else document.styleSheets[f].addRule&&document.styleSheets[document.styleSheets.length-1].addRule(".pos"+(g+this.build_offset)," left:"+(b-1)*this.pix_dim+"px;top:"+(this.build_rowNum-1)*this.pix_dim+"px")}catch(h){BigBlock.Log.display(h.name+": "+h.message)}}var i;
try{if(document.styleSheets[f].cssRules)i=document.styleSheets[f].cssRules.length;else if(document.styleSheets[f].rules)i=document.styleSheets[f].rules.length;else throw Error("Err: GB002");}catch(j){BigBlock.Log.display(j.name+": "+j.message)}this.build_offset=i;BigBlock.Loader.update({total:Math.round(i/(Math.round(a/BigBlock.Grid.pix_dim)*Math.round(e/BigBlock.Grid.pix_dim))*100)});end_time=(new Date).getTime();return i<Math.round(a/BigBlock.Grid.pix_dim)*Math.round(e/BigBlock.Grid.pix_dim)?false:
true},buildCharStyles:function(){var b=document.styleSheets.length-1;try{if(document.styleSheets[b].insertRule){document.styleSheets[b].insertRule(".text_bg{background-color: transparent;}",0);document.styleSheets[b].insertRule(".char{width : 1px;height : 1px;position: absolute;float: left;}",0)}else if(document.styleSheets[b].addRule){document.styleSheets[b].addRule(".text_bg","background-color: transparent");document.styleSheets[b].addRule(".char","width : 1px;height : 1px;position: absolute;float: left")}else throw Error("Errr: GBCS001");
for(var a=1,e=0;e<this.char_width*this.char_height;e++){if(document.styleSheets[b].insertRule)document.styleSheets[b].insertRule(".char_pos"+(e+1)+"{left:"+a+"px;top:"+(Math.floor(e/8)+1)+"px;}",0);else if(document.styleSheets[b].addRule)document.styleSheets[b].addRule(".char_pos"+(e+1),"left:"+a+"px;top:"+(Math.floor(e/8)+1)+"px;");else throw Error("Err: GBCS001");if(a+1<=this.char_width)a++;else a=1}}catch(c){BigBlock.Log.display(c.name+": "+c.message)}return true}}}();
...
BigBlock.Loader=function(){return{id:"loader",class_name:"loader_container",style:{position:"absolute",backgroundColor:"transparent",border:"1px solid #aaa",padding:"2px",textAlign:"center",color:"#fff"},width:100,height:10,format:"percentage",total:0,msg:"&nbsp;",bar_color:"#999",add:function(){try{if(window.innerWidth){this.style.left=window.innerWidth/2-this.width/2+"px";this.style.top=window.innerHeight/2-this.height/2+"px"}else if(document.documentElement.clientWidth){this.style.left=document.documentElement.clientWidth/
2-this.width/2+"px";this.style.top=document.documentElement.clientHeight/2-this.height/2+"px"}else{this.style.left="350px";this.style.top="195px";throw Error("Err: LA001");}}catch(b){BigBlock.Log.display(b.name+": "+b.message)}this.style.width=this.width+"px";this.style.height=this.height+"px";document.body.innerHTML="<div id='"+this.id+"' class='"+this.class_name+"'><div id='"+this.id+"_bar' class='"+this.class_name+"_bar' style='background-color:"+this.bar_color+";height:"+this.height+"px;'></div></div>";
for(var a in this.style)if(this.style.hasOwnProperty(a))document.getElementById(this.id).style[a]=this.style[a]},update:function(b){if(typeof b!="undefined")if(typeof b.total!="undefined")this.total=b.total;b=this.total/100*100;document.getElementById(this.id+"_bar").style.width=b+"px"}}}();
...
BigBlock.RenderMgr=function(){return{renderPix:function(){var b=BigBlock.GridActive.quads;for(var a in b)if(b.hasOwnProperty(a)){var e=document.getElementById(b[a].id);if(e.hasChildNodes()){pixNodes=e.childNodes;for(e=0;e<pixNodes.length;e++)pixNodes[e].setAttribute("class","pix color")}}document.getElementById("div_render_check")&&document.getElementsByTagName("body")[0].removeChild(document.getElementById("div_render_check"));var c,f,g;b=0;var h;for(a=0;a<BigBlock.Sprites.length;a++){if(BigBlock.Sprites[a].render!==
0&&BigBlock.Sprites[a].state!="dead"){e=0;if(typeof BigBlock.Sprites[a].img!="undefined"&&typeof BigBlock.Sprites[a].img.pix!="undefined")for(e in BigBlock.Sprites[a].img.pix){g=BigBlock.Sprites[a].img.pix[e].i;var i=g+BigBlock.Sprites[a].pix_index;if(i!==false){c=BigBlock.Sprites[a].img.pix[e].c;var j=document.createElement("div");if(BigBlock.Sprites[a].render_static)switch(BigBlock.Sprites[a].alias){case "Char":f=BigBlock.getPixLoc("x",BigBlock.Sprites[a].x,g);g=BigBlock.getPixLoc("y",BigBlock.Sprites[a].y,
g);if(f>=0&&f<BigBlock.GridStatic.width&&g>=0&&g<BigBlock.GridStatic.height){i=BigBlock.getPixIndex(f,g);j.setAttribute("class","pix text_bg pos"+i);h=BigBlock.Sprites[a].char_pos;for(k=0;k<h.length;k++){i=document.createElement("div");i.setAttribute("class","char char_pos"+h[k].p+" color"+c);j.appendChild(i)}j.setAttribute("id",BigBlock.Sprites[a].word_id);document.getElementById(BigBlock.RenderMgr.getQuad("text",f,g)).appendChild(j)}break;default:f=BigBlock.getPixLoc("x",BigBlock.Sprites[a].x,g);
g=BigBlock.getPixLoc("y",BigBlock.Sprites[a].y,g);if(f>=0&&f<BigBlock.GridStatic.width&&g>=0&&g<BigBlock.GridStatic.height){i=BigBlock.getPixIndex(f,g);j.setAttribute("class","pix color"+c+" pos"+i);j.setAttribute("id","_static");j.setAttribute("alias",BigBlock.Sprites[a].alias);document.getElementById(BigBlock.RenderMgr.getQuad("static",f,g)).appendChild(j)}break}else{h=document.getElementById(b);f=BigBlock.getPixLoc("x",BigBlock.Sprites[a].x,g);g=BigBlock.getPixLoc("y",BigBlock.Sprites[a].y,g);
if(f>=0&&f<BigBlock.GridActive.width&&g>=0&&g<BigBlock.GridActive.height){i=BigBlock.getPixIndex(f,g);if(h){h.setAttribute("class","pix color"+c+" pos"+i);c=h.parentNode.getAttribute("id");f=BigBlock.RenderMgr.getQuad("active",f,g);if(c!=f){document.getElementById(c).removeChild(h);document.getElementById(f).appendChild(h)}}else{j.setAttribute("id",b);j.setAttribute("class","pix color"+c+" pos"+i);document.getElementById(BigBlock.RenderMgr.getQuad("active",f,g)).appendChild(j)}}}}else if(h=document.getElementById(b)){c=
h.parentNode.getAttribute("id");document.getElementById(c).removeChild(h)}b++}BigBlock.Sprites[a].render--}if(BigBlock.Sprites[a].render_static&&typeof BigBlock.Sprites[a].img!="undefined"){BigBlock.Sprites[a].render_static=false;BigBlock.Sprites[a].die()}}a=document.createElement("div");a.setAttribute("id","div_render_check");document.getElementsByTagName("body")[0].appendChild(a)},getQuad:function(b,a,e){var c;switch(b){case "active":c=BigBlock.GridActive;break;case "static":c=BigBlock.GridStatic;
break;case "text":c=BigBlock.GridText;break}if(a<BigBlock.Grid.width/2&&e<BigBlock.Grid.height/2)return c.quads[0].id;if(a>=BigBlock.Grid.width/2&&e<BigBlock.Grid.height/2)return c.quads[1].id;if(a<BigBlock.Grid.width/2&&e>=BigBlock.Grid.height/2)return c.quads[2].id;if(a>=BigBlock.Grid.width/2&&e>=BigBlock.Grid.height/2)return c.quads[3].id},clearScene:function(b,a){try{if(typeof b!="undefined")if(typeof b!="function")throw Error("Err: RMCS001");else b()}catch(e){BigBlock.Log.display(e.name+": "+
e.message)}var c,f,g;if(typeof BigBlock.GridStatic!="undefined"){c=BigBlock.GridStatic.quads;for(f in c)if(c.hasOwnProperty(f)){g=document.getElementById(c[f].id);if(g.hasChildNodes())for(;g.firstChild;)g.removeChild(g.firstChild)}BigBlock.GridStatic.setStyle("backgroundColor","transparent")}if(typeof BigBlock.GridText!="undefined"){c=BigBlock.GridText.quads;for(f in c)if(c.hasOwnProperty(f)){g=document.getElementById(c[f].id);if(g.hasChildNodes())for(;g.firstChild;)g.removeChild(g.firstChild)}BigBlock.GridText.setStyle("backgroundColor",
"transparent")}if(typeof BigBlock.GridActive!="undefined"){c=BigBlock.GridActive.quads;for(f in c)if(c.hasOwnProperty(f)){g=document.getElementById(c[f].id);if(g.hasChildNodes())for(;g.firstChild;)g.removeChild(g.firstChild)}BigBlock.GridActive.setStyle("backgroundColor","transparent")}if(typeof BigBlock.Sprites!="undefined")BigBlock.Sprites=[];try{if(typeof a!="undefined")if(typeof a!="function")throw Error("Err: RMCS002");else a()}catch(h){BigBlock.Log.display(h.name+": "+h.message)}}}}();
BigBlock.ScreenEvent=function(){return{create:function(b){this.alias="ScreenEvent";try{if(typeof b=="undefined")throw Error("Err: SEC001");else{this.event_start=(new Date).getTime();this.event_time=this.event_end=0;this.event_buffer=typeof b.event_buffer!="undefined"?b.event_buffer:200;this.mouseup_event=typeof b.mouseup!="undefined"?b.mouseup:function(e){BigBlock.ScreenEvent.event_end=(new Date).getTime();BigBlock.ScreenEvent.event_time=BigBlock.ScreenEvent.event_end-BigBlock.ScreenEvent.event_start;
e.preventDefault&&e.preventDefault();e.stopPropagation&&e.stopPropagation();document.removeEventListener&&document.removeEventListener("touchstart",BigBlock.ScreenEvent.touch_event,false);if(BigBlock.ScreenEvent.event_time>BigBlock.ScreenEvent.event_buffer){BigBlock.ScreenEvent.frameAdvance(e,e.clientX,e.clientY);typeof BigBlock.InputFeedback!="undefined"&&BigBlock.Timer.input_feedback===true&&BigBlock.InputFeedback.display(e.clientX,e.clientY);if(typeof BigBlock.TapTimeout!="undefined"){BigBlock.TapTimeout.stop();
BigBlock.TapTimeout.start()}BigBlock.ScreenEvent.event_start=(new Date).getTime()}};if(document.addEventListener)document.addEventListener("mouseup",this.mouseup_event,false);else window.attachEvent&&document.attachEvent("onmouseup",this.mouseup_event);this.mousemove_event=typeof b.mousemove!="undefined"?b.mousemove:function(e){e.preventDefault()};if(document.addEventListener)document.addEventListener("mousemove",this.mousemove_event,false);else window.attachEvent&&document.attachEvent("onmousemove",
this.mousemove_event);this.mousedown_event=typeof b.mousedown!="undefined"?b.mousedown:function(e){e.preventDefault()};if(document.addEventListener)document.addEventListener("mousedown",this.mousedown_event,false);else window.attachEvent&&document.attachEvent("onmousedown",this.mousedown_event);this.touch_event_start=typeof b.touchstart!="undefined"?b.touchstart:function(e){BigBlock.ScreenEvent.event_end=(new Date).getTime();BigBlock.ScreenEvent.event_time=BigBlock.ScreenEvent.event_end-BigBlock.ScreenEvent.event_start;
e.preventDefault();this.touch=e.touches[0];document.removeEventListener("click",BigBlock.ScreenEvent.click_event,false);if(BigBlock.ScreenEvent.event_time>BigBlock.ScreenEvent.event_buffer){BigBlock.ScreenEvent.frameAdvance(e,this.touch.clientX,this.touch.clientY);typeof BigBlock.InputFeedback!="undefined"&&BigBlock.Timer.input_feedback===true&&BigBlock.InputFeedback.display(this.touch.clientX,this.touch.clientY);if(typeof BigBlock.TapTimeout!="undefined"){BigBlock.TapTimeout.stop();BigBlock.TapTimeout.start()}BigBlock.ScreenEvent.event_start=
(new Date).getTime()}};document.addEventListener&&document.addEventListener("touchstart",this.touch_event_start,false);this.touch_event_move=typeof b.touchmove!="undefined"?b.touchmove:function(e){e.preventDefault()};document.addEventListener&&document.addEventListener("touchmove",this.touch_event_move,false);this.touch_event_end=typeof b.touchend!="undefined"?b.touchend:function(e){e.preventDefault();e.stopPropagation()};document.addEventListener&&document.addEventListener("touchend",this.touch_event_end,
false);this.frameAdvance=typeof b.frameAdvance!="undefined"?b.frameAdvance:function(){switch(BigBlock.curentFrame){case 1:typeof BigBlock.TapTimeout!="undefined"&&BigBlock.TapTimeout.die();break}};if(typeof b.inputFeedback!="undefined")this.inputFeedback=b.inputFeedback}}catch(a){BigBlock.Log.display(a.name+": "+a.message)}}}}();
...
"div.pix":"float:left;width:"+BigBlock.Grid.pix_dim+"px;height:"+BigBlock.Grid.pix_dim+"px;position:absolute;"};try{if(document.styleSheets[a].insertRule)for(e in c)c.hasOwnProperty(e)&&document.styleSheets[a].insertRule(e+" {"+c[e]+"}",0);else if(document.styleSheets[a].addRule)for(e in c)c.hasOwnProperty(e)&&document.styleSheets[a].addRule(e,c[e],0);else throw Error("Err: TP001");}catch(f){BigBlock.Log.display(f.name+": "+f.message)}BigBlock.GridActive=BigBlock.clone(BigBlock.Grid);BigBlock.GridActive.configure({quads:[{id:"qA_tl",
left:BigBlock.Grid.width/2,top:BigBlock.Grid.height/2,zIndex:1},{id:"qA_tr",left:0,top:BigBlock.Grid.height/2,zIndex:1},{id:"qA_bl",left:BigBlock.Grid.width/2,top:0,zIndex:1},{id:"qA_br",left:0,top:0,zIndex:1}],styles:this.grid_active_styles});BigBlock.GridStatic=BigBlock.clone(BigBlock.Grid);BigBlock.GridStatic.configure({quads:[{id:"qS_tl",left:BigBlock.Grid.width/2,top:BigBlock.Grid.height/2,zIndex:-10},{id:"qS_tr",left:0,top:BigBlock.Grid.height/2,zIndex:-1},{id:"qS_bl",left:BigBlock.Grid.width/
2,top:0,zIndex:-1},{id:"qS_br",left:0,top:0,zIndex:-1}],styles:this.grid_static_styles});BigBlock.GridText=BigBlock.clone(BigBlock.Grid);BigBlock.GridText.configure({quads:[{id:"qT_tl",left:BigBlock.Grid.width/2,top:BigBlock.Grid.height/2,zIndex:20},{id:"qT_tr",left:0,top:BigBlock.Grid.height/2,zIndex:2},{id:"qT_bl",left:BigBlock.Grid.width/2,top:0,zIndex:2},{id:"qT_br",left:0,top:0,zIndex:2}],styles:this.grid_text_styles});if(this.input_feedback===true){input_feedback=this.input_feedback_grad!==
false?this.input_feedback_grad:"rgb(255, 255, 255);rgb(239, 239, 239);rgb(216, 216, 216);rgb(189, 189, 189);rgb(159, 159, 159);rgb(127, 127, 127);rgb(96, 96, 96);rgb(66, 66, 66);rgb(39, 39, 39);rgb(16, 16, 16)";e={classes:[{name:"input_feedback",val:input_feedback.split(";")}]};BigBlock.Color.add(e);BigBlock.InputFeedback.configure({className:"input_feedback"})}typeof BigBlock.ScreenEvent.alias=="undefined"&&BigBlock.ScreenEvent.create({});this.add_char_loader()},add_char_loader:function(){BigBlock.Loader.add();
BigBlock.Loader.update();var a=document.createElement("style");a.setAttribute("type","text/css");document.getElementsByTagName("head")[0].appendChild(a);this.build_char()},build_char:function(){BigBlock.GridActive.buildCharStyles()?this.add_color_loader():setTimeout(this.build_char,1)},add_color_loader:function(){BigBlock.Loader.add();BigBlock.Loader.update();var a=document.createElement("style");a.setAttribute("type","text/css");document.getElementsByTagName("head")[0].appendChild(a);this.build_color()},
build_color:function(){BigBlock.Color.build()?BigBlock.Timer.add_grid_loader():setTimeout(BigBlock.Timer.build_color,1)},add_grid_loader:function(){BigBlock.Loader.add();BigBlock.Loader.update();var a=document.createElement("style");a.setAttribute("type","text/css");document.getElementsByTagName("head")[0].appendChild(a);BigBlock.Timer.build_grid()},build_grid:function(){if(BigBlock.GridActive.build()){document.body.innerHTML="";BigBlock.GridActive.add();BigBlock.GridStatic.add();BigBlock.GridText.add();
for(var a in BigBlock.Sprites)if(BigBlock.Sprites.hasOwnProperty(a))BigBlock.Sprites[a].state="start";BigBlock.Log.display((new Date).getTime()-b+"ms");delete BigBlock.GridActive.build;delete BigBlock.GridActive.add;delete BigBlock.GridStatic.add;delete BigBlock.GridStatic.build;delete BigBlock.GridText.add;delete BigBlock.GridText.build;delete BigBlock.Loader;delete BigBlock.Timer.add_color_loader;delete BigBlock.Timer.build_color;delete BigBlock.Timer.add_grid_loader;delete BigBlock.Timer.build_grid;
delete BigBlock.Timer.play;if(BigBlock.Timer.tap_timeout){if(typeof BigBlock.CharPresets.getChar!="function")BigBlock.CharPresets=BigBlock.CharPresets.install();BigBlock.TapTimeout.start(BigBlock.Timer.tap_timeout_params)}if(BigBlock.Timer.app_header!==false)BigBlock.Header.add(BigBlock.GridActive.x,BigBlock.GridActive.y,typeof BigBlock.Timer.app_header_styles.backgroundColor=="undefined"?BigBlock.Timer.document_body_style.backgroundColor:BigBlock.Timer.app_header_styles.backgroundColor);if(BigBlock.Timer.app_footer!==
false)BigBlock.Footer.add(BigBlock.GridActive.x,BigBlock.GridActive.y+BigBlock.GridActive.height,typeof BigBlock.Timer.app_footer_styles.backgroundColor=="undefined"?BigBlock.Timer.document_body_style.backgroundColor:BigBlock.Timer.app_footer_styles.backgroundColor);BigBlock.Timer.before_play!==false&&typeof BigBlock.Timer.before_play=="function"&&BigBlock.Timer.before_play();BigBlock.Timer.run()}else setTimeout(BigBlock.Timer.build_grid,1)},run:function(){for(var a=0;a<BigBlock.Sprites.length;a++)typeof BigBlock.Sprites[a]==
"object"&&typeof BigBlock.Sprites[a].run=="function"&&BigBlock.Sprites[a].run();BigBlock.Timer.cleanUpSprites();this.end=(new Date).getTime();this.time=this.end-this.start;if(this.time<=this.frame_rate+1)this.buffer=setTimeout("BigBlock.Timer.render()",this.frame_rate-this.time);else BigBlock.Timer.render()},cleanUpSprites:function(){for(var a=0;a<this.sprites_to_kill.length;a++)if(typeof this.sprites_to_kill[a]!="undefined"&&typeof BigBlock.Sprites[this.sprites_to_kill[a]]!="undefined")BigBlock.Sprites[this.sprites_to_kill[a]].state=
"dead";a=false;for(var e=0;e<BigBlock.Sprites.length;e++)if(BigBlock.Sprites[e].state=="dead"){typeof BigBlock.Sprites[e].afterDie=="function"&&BigBlock.Sprites[e].afterDie();BigBlock.Sprites.splice(e,1);a=true}if(a)for(a=0;a<BigBlock.Sprites.length;a++)BigBlock.Sprites[a].index=a;this.sprites_to_kill=[]},render:function(){this.clock++;BigBlock.RenderMgr.renderPix(BigBlock.Sprites);this.buffer=setTimeout("BigBlock.Timer.run()",this.frame_rate);this.start=(new Date).getTime()},killObject:function(a){if(typeof a!=
...
"_char",value:a.value,x:a.x,y:a.y,className:a.className});else for(c=0;c<a.value.length;c++)BigBlock.CharSimple.create({word_id:a.word_id+"_char",value:a.value[c],x:a.x+c*BigBlock.Grid.pix_dim,y:a.y,className:a.className})}catch(f){BigBlock.Log.display(f.name+": "+f.message)}BigBlock.Sprites.push(a)}}}();