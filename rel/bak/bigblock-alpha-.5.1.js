...
BigBlock.Grid = (function () {
	...
	return {
		...
		add: function(){
			...
			for (var i = 0; i < 4; i++) {
				try {
					if (document.styleSheets[css_i].insertRule) { // Mozilla
						document.styleSheets[document.styleSheets.length - 1].insertRule("div#" + this.quads[i].id + " {background-color:transparent;width: " + BigBlock.Grid.width/2 + "px;height: " + BigBlock.Grid.height/2 + "px;position: absolute;left: " + ((win_width / 2) - (this.quads[i].left)) + "px;top: " + ((win_height / 2) - (this.quads[i].top)) + "px;z-index: " + this.quads[i].zIndex + "}", 0);
					} else if (document.styleSheets[css_i].addRule) { // IE
						document.styleSheets[document.styleSheets.length - 1].addRule("div#" + this.quads[i].id, "background-color:transparent;width: " + BigBlock.Grid.width/2 + "px;height: " + BigBlock.Grid.height/2 + "px;position: absolute;left: " + ((win_width / 2) - (this.quads[i].left)) + "px;top: " + ((win_height / 2) - (this.quads[i].top)) + "px;z-index: " + this.quads[i].zIndex);
					} else {
			...
			this.x = ((win_width / 2) - (this.quads[0].left)); // set the Grid x, y to the first quad's x, y
			this.y = ((win_height / 2) - (this.quads[0].top));
		...
		buildCharStyles : function () {
			...
			try {
				if (document.styleSheets[css_i].insertRule) { // Mozilla
					...
					document.styleSheets[css_i].insertRule(".char{width : 1px;height : 1px;position: absolute;float: left;}", 0);					
				} else if (document.styleSheets[css_i].addRule) { // IE
					document.styleSheets[css_i].addRule(".text_bg", "background-color: transparent");
					document.styleSheets[css_i].addRule(".char", "width : 1px;height : 1px;position: absolute;float: left");
				} else {
	...
	return {
		...
		play : function(params) { // called after all objects are ready
			...
			var css = {
				...
				'div.pix' : 'float:left;width:' + BigBlock.Grid.pix_dim + 'px;height:' + BigBlock.Grid.pix_dim + 'px;position:absolute;'
			};
