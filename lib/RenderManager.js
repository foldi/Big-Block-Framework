	...
	return {
		...
		renderActiveBlock: function (id, obj, color, offset) {
			...
			if (BigBlock.ptInsideRect(x, y, 0, BigBlock.Grid.width, 0, BigBlock.Grid.height)) { // check that block is on screen
				...
				var d = document.getElementById(id); // dom method
										
				if (!d) { // if this div does not exist, create it
					
					var child = document.createElement("div"); // dom method; create a div element
					...
					if (this.is_cleaned || obj.is_position_updated || obj.is_anim_updated || obj.is_color_updated) { // 
						...
						var parent = d.parentNode.getAttribute("id");
	
						var target = BigBlock.RenderMgr.getQuad('active', x, y);												 											
		...
		renderStaticBlock: function (obj, color, offset) {
			var x = BigBlock.getPixLoc('x', obj.x, offset); // get global x, y coords based on parent Block's coords
			var y = BigBlock.getPixLoc('y', obj.y, offset);
	
			if (BigBlock.ptInsideRect(x, y, 0, BigBlock.Grid.width, 0, BigBlock.Grid.height)) { // check that block is on screen
				
				var index = BigBlock.getPixIndex(x, y);
		...
		renderChar: function (id, obj, color, offset) {
			...
			if (BigBlock.ptInsideRect(x, y, 0, BigBlock.Grid.width, 0, BigBlock.Grid.height) && y < BigBlock.GridStatic.height) { // check that block is on screen
				...
				var char_pos = obj.char_pos; // get positions of all divs in the character
				
				for (var k = 0; k < char_pos.length; k++) {
					var char_div = document.createElement("div"); // dom method; create a div for each block in the letter
					
					var gl_limit = BigBlock.Grid.blk_dim * 4; // check for glass													
		...
		runRateTest : function (start) {
			var time = new Date().getTime() - start; // calculate how long the run took in milliseconds
			var test_interval = BigBlock.Timer.frame_rate_test_interval;
			
			this.render_rate_test_array[this.render_rate_test_array.length] = time;
			
			if (BigBlock.Timer.clock%test_interval == 0) {
				var total = 0;
				for (var t = 0; t < test_interval; t++) {
					total += this.render_rate_test_array[t];
					
				}
				var ms = total/test_interval;
