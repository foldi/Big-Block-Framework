	...
	return {
		...
		renderPix : function (start) {
			...
			for (i = 0; i < BigBlock.Blocks.length; i++) {
				 ...
				 * immediately deleted from the Block array. You cannot kill
				 * static sprites.
				 * 
				 */
				
				
				if (BigBlock.Blocks[i].render !== 0 && BigBlock.Blocks[i].state != 'dead' && typeof(BigBlock.Blocks[i].img) != 'undefined' && typeof(BigBlock.Blocks[i].img.pix) != 'undefined') {
										
					for (y = 0; y < BigBlock.Blocks[i].img.pix.length; y++) { // loop thru blocks attached to this Block
							
						// render pixel
						var pix_index_offset = BigBlock.Blocks[i].img.pix[y].i;
						var pix_index = pix_index_offset + BigBlock.Blocks[i].pix_index; 
						
						if (pix_index !== false) { // check that block is on screen; index should correspond to a location in the grid
							var color = BigBlock.Blocks[i].img.pix[y].c;
							
							var child = document.createElement("div");
							
							if (BigBlock.Blocks[i].render_static) {
							
								switch (BigBlock.Blocks[i].alias) {
									case 'Char':
										
										pix_x = BigBlock.getPixLoc('x', BigBlock.Blocks[i].x, pix_index_offset); // get global x, y coords based on parent sprite's coords
										pix_y = BigBlock.getPixLoc('y', BigBlock.Blocks[i].y, pix_index_offset);
										...
										if (pix_x >= 0 && pix_x < BigBlock.GridStatic.width && pix_y >= 0 && pix_y < BigBlock.GridStatic.height) { // check that block is on screen
											...
											var char_pos = BigBlock.Blocks[i].char_pos; // get positions of all divs in the character
											
											for (var k = 0; k < char_pos.length; k++) {
												var char_div = document.createElement("div");
												
												var gl_limit = BigBlock.Grid.blk_dim * 4;													
												
												if (char_pos[k].p < gl_limit && BigBlock.Blocks[i].glass === true) {
											...
											child.setAttribute('name', BigBlock.Blocks[i].word_id);
											child.setAttribute('id', BigBlock.getUniqueId());
											
											if (BigBlock.Blocks[i].url !== false) { // if a url is attached to the char, add an event listener
												child.style['backgroundColor'] = BigBlock.Blocks[i].link_color;
											}
									...
									default:
										...
										if (pix_x >= 0 && pix_x < BigBlock.GridStatic.width && pix_y >= 0 && pix_y < BigBlock.GridStatic.height) { // check that block is on screen
											...
											child.setAttribute('name', BigBlock.Blocks[i].alias);	
					...
					BigBlock.Blocks[i].render--; // decrement the render counter
				}
				if (BigBlock.Blocks[i].render_static && typeof(BigBlock.Blocks[i].img) != 'undefined') { // if this is a static object and the object has its img property
					BigBlock.Blocks[i].render_static = false; // MUST set the static property = 0; static sprites will not be deleted
					BigBlock.Blocks[i].destroy(); // kill the sprite
				}				
			}
