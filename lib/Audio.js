	...
	return {
		...
		add: function(id, path, loop) {
			...
			if (supported) {
				...
				for (var f = 0; f < this.format.length; f++) { // loop thru formats to find the first that this browser will play
					
					var mime_type = BigBlock.getMimeTypeFromFileExt(this.format[f]); // get mime-type
					
					var audio = new Audio(path + id + "." + this.format[f]); // create audio element
					if (loop) {
						audio.loop = loop;
					}
					
					var supported_mime_type = ""; // loop thru mime_type
					
					for (var i = 0; i < mime_type.length; i++) {
