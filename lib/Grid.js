	...
	return {
		...
		build : function () {
			...
			for (i = 0, max = ((viewport_width * viewport_height) / build_section_total) * 4; i < max; i += 1) { // loop thru row; adding multiplier speeds up build, reduces the number of times the call is made
				...
				if (s.insertRule) { 
					s.insertRule("." + BigBlock.CSSid_position + (i + this.build_offset) + " {left:" + ((colNum - 1) * blk_dim) + "px;top:" + ((this.build_rowNum - 1) * blk_dim) + "px;}", 0); // setup pos rules
				} else if (s.addRule) { // IE
					s.addRule("." + BigBlock.CSSid_position + (i + this.build_offset), " left:" + ((colNum - 1) * blk_dim) + "px;top:" + ((this.build_rowNum - 1) * blk_dim) + "px"); // setup pos rules
				}
