...
BigBlock.Grid = (function () {
	...
	return {
		...
		appendRule: function (sheet, selector, rule) {
			if (sheet.insertRule) {
				this.appendRule = function (sheet, selector, rule) {
			...
			this.appendRule(sheet, selector, rule);
		},
		/**
		...
		build : function () {
			...
			for (i = 0, max = ((viewport_width * viewport_height) / build_section_total) * 4; i < max; i += 1) { // loop thru row; adding multiplier speeds up build, reduces the number of times the call is made
				...
				this.appendRule(sheet, BigBlock.CSSid_position + (i + this.build_offset), "left:" + ((colNum - 1) * blk_dim) + "px;top:" + ((this.build_rowNum - 1) * blk_dim) + "px;");
		...
		buildCharStyles : function () {
			...
			this.appendRule(sheet, BigBlock.CSSid_text_bg, "background-color: transparent;");
			this.appendRule(sheet, BigBlock.CSSid_char, "width: 1px;height: 1px;position: absolute;float: left;line-height: 0px;font-size: 1%;");
						
			col_count = 1;
			for (i = 0, max = this.char_width * this.char_height; i < max; i += 1) {
				
				this.appendRule(sheet, BigBlock.CSSid_char_pos + (i + 1), "left:" + col_count + "px;top:" + (Math.floor((i/8))+1) + "px;");
