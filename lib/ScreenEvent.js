...
BigBlock.ScreenEvent = (function () {
	...
	var event_buffer = 200; // default event buffer in milliseconds
		
	var inputFeedback = function (evt_x, evt_y) {
	...
	var removeListensers = function (event_name, event) {
		for (var i = 0; i < BigBlock.GridActive.quads.length; i++) { // remove listeners from quads
	...
	var checkButtonAction = function (evt_x, evt_y, event) {
	...
	var checkBlockAction = function (evt_x, evt_y, event) {
		// loops thru Block and looks for action_inputs
		for (var i = 0; i < BigBlock.BlocksKeys.length; i++) {
			if (typeof(BigBlock.Blocks[BigBlock.BlocksKeys[i]]) != 'undefined' && typeof(BigBlock.Blocks[BigBlock.BlocksKeys[i]].action_input) != 'undefined' && BigBlock.Blocks[BigBlock.BlocksKeys[i]].action_input !== false && typeof(BigBlock.Blocks[BigBlock.BlocksKeys[i]].action_input) == 'function') {
				var x = BigBlock.Blocks[BigBlock.BlocksKeys[i]].x;
				var y = BigBlock.Blocks[BigBlock.BlocksKeys[i]].y;
				var width = BigBlock.Blocks[BigBlock.BlocksKeys[i]].width;
				var height = BigBlock.Blocks[BigBlock.BlocksKeys[i]].height;											
	...
	var checkInputBlock = function () {
		if (BigBlock.inputBlock === true) return false; // check to block user input
		return true;		
	};
	...
	return {
		...
		create: function (params) {
			...
			this.mouseup_event = function (event) {
				...
				if (!BigBlock.ScreenEvent.checkInputBlock()) return false; // check if input is blocked
				...
				if (BigBlock.ScreenEvent.event_time > BigBlock.ScreenEvent.event_buffer && !BigBlock.Timer.is_paused) {
					...
					if (!BigBlock.ScreenEvent.checkButtonAction(evt_loc.x, evt_loc.y, event)) return false;

					if (!BigBlock.ScreenEvent.checkBlockAction(evt_loc.x, evt_loc.y, event))  return false;																																							
